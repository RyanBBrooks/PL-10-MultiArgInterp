...
(module+ test
  ...
  (test (interp (parse `{let {[x : num 5]}
                ...
                mt-env)
        (numV 10))
  (test (interp (parse `{{lambda {} 999}})
                mt-env)
        (numV 999))
  (test (interp (parse `{{lambda {[x : num] [y : num] [z : bool]} {if z x y}} 5 -10 true})
                mt-env)
        (numV 5))
  (test (interp (parse `{{lambda {[x : num] [y : num] [z : bool]} {if z x y}} 5 -10 false})
                mt-env)
        (numV -10))
  (test (interp (parse `{let {[x : num 5]}
                          {let {[x : num {+ 1 x}]}
...
(module+ test
  ...
  (test (typecheck (parse `{lambda {[x : num] [a : bool]} {lambda {[y : bool]} x}}) mt-env)
        (arrowT (list (numT) (boolT)) (arrowT (list (boolT))  (numT))))
  (test (typecheck (parse `{lambda {[x : num] [a : bool]} {lambda {[y : bool] [b : (-> num)]} x}}) mt-env)
        (arrowT (list (numT) (boolT)) (arrowT (list (boolT)(arrowT empty (numT)))  (numT))))
  (test (typecheck (parse `{lambda {} 12}) mt-env)
        (arrowT empty (numT)))
  (test (typecheck (parse `{lambda {[x : num] [a : bool] [q : bool]} {lambda {[y : bool] [b : (-> num)]} x}}) mt-env)
        (arrowT (list (numT) (boolT) (boolT)) (arrowT (list (boolT)(arrowT empty (numT)))  (numT))))
  ...
  (test/exn (typecheck (parse `{* {lambda {[x : num]} x} 1})
                       mt-env)
            "no type")
  (test/exn (typecheck (parse `{{lambda {[x : num] [y : bool]} {if y x 4}} 1 1})
                       mt-env)
            "no type")
  (test (typecheck (parse `{{lambda {[x : num] [y : bool]} {if y x 4}} 1 false})
                       mt-env)
            (numT)))